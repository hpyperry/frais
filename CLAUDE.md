...
- **LLM client layer**: `llm/` package — protocol-agnostic ABC (`LLMClient`) with `OpenAICompatibleClient` as the OpenAI-protocol base and `DeepSeekOpenAIClient` / `DeepSeekAnthropicClient` for DeepSeek's two API protocols. Factory `get_client(config, protocol)` selects by `(provider_id, protocol)` from `_CLIENT_MAP`. Chat defaults to the OpenAI protocol; the Anthropic protocol is reserved for future features (e.g. server-side web search). Each provider subclass injects thinking control via its own mechanism (`extra_body` for OpenAI, `thinking` param for Anthropic). `summarize_candidate()` is a standalone function in `commands/summarize.py`, not a Client method.
