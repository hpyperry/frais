...
_CLIENT_MAP: dict[tuple[str, str], type[LLMClient]] = {
    ...
    ("mimo", "openai"): MiMoClient,
}


def get_client(config: ProviderConfig, protocol: str | None = None) -> LLMClient:
    """Create the appropriate LLM client for a provider and protocol.

    Args:
        config: Provider configuration with API key, model, protocol.
        protocol: API protocol to use (e.g. "openai", "anthropic").
                   When None, reads from config.protocol. Default None.
